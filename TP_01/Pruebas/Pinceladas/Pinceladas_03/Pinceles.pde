class Pinceles 
{
  ...
  float posX=0, posY=0, angulo, ancho, alto;
  Pinceles ()
  {
    cantPinceles=7;
    cargaPincelesPrueba();
  ...
  Pinceles (String rutaPincel_, float ancho_, float alto_)
  {
    ancho= ancho_;
    alto= alto_;
    setPincel(rutaPincel_);
  }

  Pinceles ( int cantPinceles_, String [] rutaPincel_)
  {
    cantPinceles=cantPinceles_;
    for (int i=0; i<cantPinceles; i++)
    {
      setPincel(rutaPincel_[i]);
    }
  }
  ...
  void dibuja()
  {
    ...
    image (pinceles.get(pincelNum), getPosicion()[0], getPosicion()[1], tamPincel+ancho, tamPincel+alto);
    popMatrix();
  }
